    ...
    for(int c=0;c<10;c++){
      ...
      while(1){
        ...
        for(int d=0;d<dim;d++){
          ...
          double mul=0;
          for(int i=0;i<data[0].size();i++){
            mul+=data[0][i][d+1]*a[i];
          }

          double mul1=0;
          for(int i=0;i<data[0].size();i++){
            mul1+=data[0][i][d+1]*data[0][i][d+1];
          }
          mul1+=lambda[l];
          weights[c][d]=mul/mul1;
          if(weights[c][d]<0)weights[c][d]=0;
        }
        //calculting Obj
        a1=0;
        for(int i1=0;i1<data[0].size();i1++){
        ...
        //cout<<"oldObj= "<<oldObj<<"newObj= "<<newObj<<" err="<<(oldObj-newObj)/oldObj<<"=======>"<<iter++<<endl;
